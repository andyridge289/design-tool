...
$tables = array("ds", "decision", "option");
...
for($i = 0; $i < count($tables); $i++)
{
	...
	{
		array_push($old, new Thing($r["id"], $r["name"], $tables[$i]));

		// TODO Need to check if the old one has already been linked, if it has then remove it
	}
}
usort($old, "cmp");
$num = count($old);
...
while($r = mysqli_fetch_array($ret))
{
	array_push($new, new Thing($r["id"], $r["name"], $r["type"]));
}
usort($new, "cmp");
...
// 	echo "<li onmouseover='over(this)' onmouseout='out(this)' onclick='oldclick(this)' id='old_$id' class='$class'>$name</li>";
// }
...
//	echo "<li onmouseover='over(this)' onmouseout='out(this)' onclick='newclick(this)' id='new_$id' class='$class'>$name</li>";
// }
