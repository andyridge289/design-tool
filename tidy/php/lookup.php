...
$q = "SELECT * FROM `thing_has_thing` AS tt 
	...
	WHERE child_id = $id AND tt.dead = 0";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail: $q<br />";
...
$q = "SELECT * FROM `thing_has_thing` AS tt
	...
	WHERE parent_id = $id AND tt.dead = 0";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail: $q<br />";
...
$q = "SELECT * FROM `thing_has_thing` AS tt 
	LEFT JOIN `thing` AS t ON t.id = tt.parent_id
	WHERE child_id = $id AND tt.dead = 1";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail: $q<br />";
...
$q = "SELECT * FROM `thing_has_thing` AS tt
	LEFT JOIN `thing` AS t ON t.id = tt.child_id
	WHERE parent_id = $id AND tt.dead = 1";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail: $q<br />";
...
$q = "SELECT * 
		FROM `thing_has_source` AS ts 
		LEFT JOIN `source` AS s
			ON ts.source_id = s.id
		WHERE ts.thing_id = $id";
$ret = $db->q($q);
if(!$ret)
{
	echo "Fail $q<br />";
